...
function UserCurrentRide() {
    ...
    const getRideData = async () => {
        try {
          ...
          if(ride_id){
              const response = await axiosRide(userToken).get(`getCurrentRide?rideId=${ride_id}`)
              const {data} = await axiosDriver().get(`driverData?driver_id=${response.data.driver_id}`);              
  ...
  return (
    <div>
      <Dialog className='bg-transparent' open={cancelModal} handler={cancelRide} placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
        {cancelModal &&
          <>
            <div className='w-full h-fit rounded-lg bg-gray-50 px-8 pt-8 flex flex-col text-center'>
              ...
              <div className='flex justify-end items-end h-fit mt-7 mb-7 gap-5'>
                <button
                  ...
                  className='w-[20%] h-10 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-all duration-300'>dismiss</button>
                <button
                  onClick={() => cancelRide()}
                  className='btn btn-xs bg-red-400 text-white hover:bg-red-600 relative right-2 rounded-full px-4 py-2 transition-colors duration-300'>Cancel ride</button>
              </div>
            </div>
          </>
        }
                        ...
                        <div className='w-full flex mt-1 h-36  items-center justify-center '>
                          <div className="avatar h-max">
                            <div className="w-36 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
                              <img className='rounded-full drop-shadow-xl' src={driverData?.driverImage} />
                            </div>
                          </div>
                        </div>
                            ...
                            <div className='px-3 mt-3 flex flex-col gap-4'>
                              ...
                              <div className=''>
                                ...
                                <div className='md:flex md:justify-between text-center md:text-left gap-1 bg-gray-100 rounded-2xl drop-shadow-lg items-center md:px-5 px-7 py-2'>
                                  <h1 className='text-[8pt] md:w-40'>Canceling a confirmed ride may affect your Go account. Please proceed with caution.</h1>
                                  <button
                                    onClick={() => setcancelModal(true)}
                                    className='btn w-[50%] ml-4 bg-red-400 text-white hover:bg-red-600 relative right-2 rounded-full px-4 py-2 transition-colors duration-300'>cancel the ride</button>
                                </div>
                              </div>
                            </div>
